...
from smrig.gui.mayawin import get_icon_path, maya_main_window
...
log = logging.getLogger("smrig.gui.widget.blendshapeloader")
filters = "Maya Scenes (*.ma *.mb);;Maya ASCII (*.ma);;Maya Binary (*.mb);;OBJ (*.obj);;FBX (*.fbx);;All Files (*.*)"


class BlendshapeLoader(QtWidgets.QDialog):

	def __init__(self, parent=maya_main_window):
		super(BlendshapeLoader, self).__init__(parent)

		# Parent widget under Maya main window
		self.setParent(parent)

		self.setWindowFlags(QtCore.Qt.Tool)
		self.setWindowTitle("RC Tools | LookDev Scene Settings")
		self.setWindowFlags(QtCore.Qt.Tool)
		...
		self.tree_file.setColumnCount(3)
		self.tree_file.setIndentation(0)
		self.tree_file.setHeaderLabels(["File Path", "", ""])
		...
		self.tree_file.header().resizeSection(3, 30)
		self.tree_file.header().resizeSection(1, 30)
		self.tree_file.hide()

		self.add_button = QtWidgets.QPushButton("Add LookDev Scene File Path")
		self.add_button.released.connect(self.add_file_item)
		...
		self.save_button = QtWidgets.QPushButton("Save LookDev Directory Setting")
	...
	def browse(self, item):
		...
		"""
		asset = env.asset.get_asset()
		base_path = pathlib.normpath(env.asset.get_rigbuild_path().replace("rigbuild", ""))
		lib_path = pathlib.normpath(os.path.join(base_path, "scenes", "Library", "Clothing_elements"))
		path = lib_path.replace(asset, "LIBRARIES")
		if not os.path.isdir(path):
			path = base_path

		file_path = cmds.fileDialog2(fileFilter=filters, dialogStyle=2, okc="Select", fm=1, dir=path)
		if file_path:
	...
	def add_file_item(self, file_path=None, unlock_normals=False, soft_normals=False, **kwargs):
		...
		self.tree_file.setItemWidget(item, 2, item.button)
