...
class DeltaMush(object):
	def __init__(self, deformer):
		...
		self.shapes = cmds.deltaMush(deformer, q=1, g=1)
		self.object_set = cmds.listConnections(self.deformer, type="objectSet")[0]
		self.components = cmds.ls(cmds.sets(self.object_set, q=1), fl=1)
		self.data = {}
	...
	def load(cls, file_path=None, method="vertexID", remap=None):
		...
		def set_data(deformer, data):
			...
			:return:
			"""
			for attr, val in data.get("attrValues").items():
		...
		if utils.check_missing_nodes(name, shapes + set_members):
			return
		...
		if len(weights) and method == "closestPoint":
			...
			result = cmds.deltaMush(set_members)
			deformer = cmds.rename(result[0], name)
...
def load(file_path, *args, **kwargs):
	...
	DeltaMush.load(file_path, method=method, remap=remap)
