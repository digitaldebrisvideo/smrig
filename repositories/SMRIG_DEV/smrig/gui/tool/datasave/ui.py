...
class DataSave(QtWidgets.QDialog):
	filters = {}
	all_filter_items = []

	def __init__(self, parent=maya_main_window):
		super(DataSave, self).__init__(parent)
	...
	def populate_filters(self):
		...
		:return:
		"""
		module_names = dataioo.types.modules.keys()
	...
	def list_deformers(self):
		...
		for node in nodes:
			for data_type in node_data_types:
				...
				if shapes:
					deformers = dataioo.utils.get_deformers(shapes[0], dtype=data_type)
					try:
	...
	def export_data(self, export_all=False):
		...
		for item in items:
			if item.node and item.data_type:
				dataioo.save_to_asset(item.data_type, node=item.node)
	...
	def export_deformation_order(self):
		...
		for node in nodes:
			dataioo.save_to_asset("deformationOrder", node=node)
	...
	def export_control_attributes(self):
		...
		:return:
		"""
		dataioo.save_to_asset("keyableAttributes")
	...
	def export_control_values(self):
		...
		:return:
		"""
		dataioo.save_to_asset("attributeValues")
